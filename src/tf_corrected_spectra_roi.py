...
# ---------------- source invariance (plug your fitted values)
SRC_Z_MODEL = "rho_c"   # "rho_c" (rho*c) or "rho"
SRC_B_DENSITY = -0.12   # <- your fitted b (from cal-only validation)

# =====================================================================
# helpers
# =====================================================================
def _psig_to_float(s: str) -> float:
    s = str(s).strip().lower()
    return float(s.replace("psig", ""))

def _pressure_sensor_correction_gain(psig: float, slope_db_per_kpa: float = -0.01) -> float:
    """
    Return the *compensation* gain [linear] that undoes a sensitivity slope of slope_db_per_kpa (dB/kPa).
    If measured A_meas = A_true * 10^(slope * kPa / 20), then A_true = A_meas * 10^(-slope * kPa / 20).
    With slope = -0.01 dB/kPa, this is >1 at positive psig (boost), as desired.
    """
    kpa = float(psig) * PSI_TO_KPA
    return 10.0 ** (-(slope_db_per_kpa) * kpa / 20.0)

def _bandpass(x: np.ndarray, fs: float, f_low: float, f_high: float, order: int = 4) -> np.ndarray:
...
def _volts_to_pa_corrected(x_volts: np.ndarray, *, channel: str, psig: float, f_high: float) -> np.ndarray:
    """Volts -> Pa then apply pressure-sensitivity *compensation* (−0.01 dB/kPa)."""
    sens_v_per_pa = CHAIN_SENS_V_PER_PA[channel.lower()]
    x_bp = _bandpass(x_volts, FS, f_low=0.1, f_high=f_high)
    x_pa = x_bp / sens_v_per_pa
    return x_pa * _pressure_sensor_correction_gain(psig)
...
def apply_frf(x: np.ndarray, fs: float, fH: np.ndarray, H: np.ndarray,
    ...
    """Apply complex FRF to time series x."""
    x = np.asarray(x, float)
...
def _rho_of_psig(psig: float, TdegC: float = 20.0) -> float:
    T = 273.15 + float(TdegC)
    P = P_ATM + float(psig) * PSI_TO_PA
    return P / (R_AIR * T)

def _c_of_T(TdegC: float = 20.0, gamma: float = 1.4) -> float:
    T = 273.15 + float(TdegC)
    return np.sqrt(gamma * R_AIR * T)

def _z_run(psig: float, TdegC: float, z_model: str) -> float:
    rho = _rho_of_psig(psig, TdegC)
    if z_model.lower() == "rho_c": return rho * _c_of_T(TdegC)
...
def source_equalizer_gain(psig: float, *, ref_psig: float, TdegC: float,
    ...
    """Constant gain to map source at psig to the reference source at ref_psig: g = (z_ref / z)^b."""
    z = _z_run(psig, TdegC, z_model)
    z_ref = _z_run(ref_psig, TdegC, z_model)
...
def build_ph_to_nc_calibration(label: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    ph1_pa = _volts_to_pa_corrected(ch1, channel="ph1", psig=psig, f_high=f_high)
    nc1_pa = _volts_to_pa_corrected(nc1, channel="nc",  psig=psig, f_high=f_high)
    ...
    ph2_pa = _volts_to_pa_corrected(ch2, channel="ph2", psig=psig, f_high=f_high)
    nc2_pa = _volts_to_pa_corrected(nc2, channel="nc",  psig=psig, f_high=f_high)
    f2, H2, g2_2 = estimate_frf(ph2_pa, nc2_pa, fs=FS)

    # coherence‑weighted fusion onto a log grid
...
def plot_raw_vs_corrected(label_pos: str,
                          *,
                          ...
                          apply_meas_sensor_corr: bool = True,
                          apply_source_equalizer: bool = False) -> None:
    """
    Steps:
      1) Build (or load) H_nc/ph(f) for the label's pressure.
      2) Read cleaned PH1/PH2; optionally apply -0.01 dB/kPa sensor compensation.
      3) Map PH -> NC with H_nc/ph.
      4) Multiply each corrected trace by a *constant* source equalizer g=(z_ref/z)^b (if enabled).
      5) Plot Welch spectra of raw vs corrected on f*Pyy/(rho^2 u_tau^4), 100–1000 Hz.
    """
    ...
    # 2) cleaned measurement + optional sensor compensation
    ...
    if apply_meas_sensor_corr:
        g_chain = _pressure_sensor_correction_gain(psig)  # compensation (>1 at positive psig)
    ...
    # 4) pressure‑invariant source equalization (constant per pressure)
    if apply_source_equalizer:
    ...
    ax.semilogx(f_raw[mraw], Y1_raw[mraw], "C0--", lw=1.0, alpha=0.9, label="PH1 raw")
    ax.semilogx(f_raw[mraw], Y2_raw[mraw], "C1--", lw=1.0, alpha=0.9, label="PH2 raw")
    ...
    ttl = (f"{label_pos} | H built at {label} | "
           f"sensor comp={'on' if apply_meas_sensor_corr else 'off'} | "
           f"source→{ref_label} via g=(z_ref/z)^b, b={SRC_B_DENSITY:+.3f}, z={SRC_Z_MODEL}")
