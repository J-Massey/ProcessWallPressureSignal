...
def plot_raw():
    ...
    with h5py.File(fps[3], 'r') as f_raw:
        for psig in psigs:
            PH1_raw_close = f_raw[f'wallp_production/{psig}/fs_noise_rejected_signals/close/PH1_Pa'][:]
            PH2_raw_close = f_raw[f'wallp_production/{psig}/fs_noise_rejected_signals/close/PH2_Pa'][:]
            PH1_raw_far = f_raw[f'wallp_production/{psig}/fs_noise_rejected_signals/far/PH1_Pa'][:]
            PH2_raw_far = f_raw[f'wallp_production/{psig}/fs_noise_rejected_signals/far/PH2_Pa'][:]
            ic(f_raw[f'wallp_production/{psig}/fs_noise_rejected_signals'].attrs.keys())
            rho = f_raw[f'wallp_production/{psig}'].attrs['rho'][()]
            u_tau = f_raw[f'wallp_production/{psig}'].attrs['u_tau'][()]
            nu = f_raw[f'wallp_production/{psig}'].attrs['nu'][()]
            ...
            ax[1].semilogx(T_plus, f * Pxx_far / norm_factor, label=psig)
