...
def plot_white(ax):
    ...
    # ph_hat_atm_lrg = wiener_forward(ph_atm_lrg, FS, f, H_atm_lrg, gamma2_atm_lrg)
    # t = np.arange(len(ph_hat_atm_lrg)) / FS
    # plot_corrected_trace_PH(t, nkd_atm_lrg, ph_atm_lrg, ph_hat_atm_lrg, f"{OUTPUT_DIR}/ph_recon_atm_lrg", "50psi")
    ...
    # ax.loglog(f, Pyy_ph_an, label='PH (all)', color=ph_colour)
    # ax.loglog(f, Pyy_ph_fn + Pyy_ph_nn, label='PH (fn+nn)', linestyle='--', color=ph_colour)
    # # plot_white(ax)
    # ax.set_xlabel("$f$ [Hz]")
...
def calibration_400_atm():
    root = 'data/20251014'
    fn = f'{root}/band_calib/400micron/atm.mat'
    OUTPUT_DIR = "figures/band_calib/400micron/atm"
    ...
    nkd = dat['channelData_NF'][:,2]
    ph1 = dat['channelData_NF'][:,0]
    ph2 = dat['channelData_NF'][:,1]
    f, Pyy_nkd = compute_spec(FS, nkd)
    f, Pyy_ph1 = compute_spec(FS, ph1)
    f, Pyy_ph2 = compute_spec(FS, ph2)
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.), sharex=True)
    ax.loglog(f, f * Pyy_nkd, label='NKD', color=nkd_colour)
    ax.loglog(f, f * Pyy_ph1, label='PH1', color=ph1_colour)
    ax.loglog(f, f * Pyy_ph2, label='PH2', color=ph2_colour)
    plot_white(ax)
    ax.set_xlabel("$f$ [Hz]")
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/400_atm_band_calib.pdf", dpi=400)
    ...
    # FRF PH→NKD (x=PH, y=NKD)
    f, H, gamma2 = estimate_frf(ph1, nkd, FS, npsg=2**10)
    np.save(f"{CAL_DIR}/H_400_atm.npy", H)
    np.save(f"{CAL_DIR}/gamma2_400_atm.npy", gamma2)
    np.save(f"{CAL_DIR}/f_400_atm.npy", f)

    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/400_atm_H", "atm")

    # Suggest cutting at 0.1Hz (high-pass) and 5kHz (low-pass)-> corresponds to what T^+?
    f_cut = 5000
    T_plus_f5000Hz = 1/f_cut * (u_tau**2)/nu
    ic(T_plus_f5000Hz)  # ~0.4

    # Now plot the TF with this cutoff, annotated
    mag = np.abs(H); ph = np.unwrap(np.angle(H))
    fig, (ax_mag, ax_ph) = plt.subplots(2, 1, sharex=True, figsize=(6, 3), dpi=600)
    ax_mag.set_title(r'$H_{\mathrm{PH-NC}}$ ($400\mu m$, atm), with suggested cutoffs')
    ...
    ax_mag.text(f_cut, 10, fr'$T^+ \approx {T_plus_f5000Hz:.1f}$', color='red', va='center', ha='right', rotation=90)
    ...
    fig.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/400_atm_H_band.pdf", dpi=600)
    plt.close()


def calibration_400_50():
    root = 'data/20251014'
    fn = f'{root}/band_calib/400micron/50psig.mat'
    OUTPUT_DIR = "figures/band_calib/400micron/50psig"
    CAL_DIR = os.path.join(root, "tf_data")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
    ...
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/400_50_band_calib.pdf", dpi=400)
    ...
    # FRF PH→NKD (x=PH, y=NKD)
    f, H, gamma2 = estimate_frf(ph1, nkd, FS, npsg=2**10)
    np.save(f"{CAL_DIR}/H_400_50.npy", H)
    np.save(f"{CAL_DIR}/gamma2_400_50.npy", gamma2)
    np.save(f"{CAL_DIR}/f_400_50.npy", f)

    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/400_50_H", "50psi")
    ...
    mag = np.abs(H); ph = np.unwrap(np.angle(H))
    fig, (ax_mag, ax_ph) = plt.subplots(2, 1, sharex=True, figsize=(6, 3), dpi=600)
    ax_mag.set_title(r'$H_{\mathrm{PH-NC}}$ ($400\mu m$, 50 psi), with suggested cutoffs')
    ax_mag.loglog(f, mag, lw=1, color='k')
    ax_mag.set_ylabel(r'$|H_{\mathrm{PH-NC}}(f)|$')
    ax_mag.set_ylim(0.1, 100)
    ax_mag.axvline(f_cut, color='red', linestyle='--', lw=1)
    ...
    fig.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/400_50_H_band.pdf", dpi=600)
    plt.close()

def calibration_400_100():
    root = 'data/20251014'
    fn = f'{root}/band_calib/400micron/100psig.mat'
    OUTPUT_DIR = "figures/band_calib/400micron/100psig"
    CAL_DIR = os.path.join(root, "tf_data")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
    ...
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/400_100_band_calib.pdf", dpi=400)
    ...
    # FRF PH→NKD (x=PH, y=NKD)
    f, H, gamma2 = estimate_frf(ph1, nkd, FS, npsg=2**10)
    np.save(f"{CAL_DIR}/H_400_100.npy", H)
    np.save(f"{CAL_DIR}/gamma2_400_100.npy", gamma2)
    np.save(f"{CAL_DIR}/f_400_100.npy", f)

    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/400_100_H", "100psi")
    ...
    mag = np.abs(H); ph = np.unwrap(np.angle(H))
    fig, (ax_mag, ax_ph) = plt.subplots(2, 1, sharex=True, figsize=(6, 3), dpi=600)
    ax_mag.set_title(r'$H_{\mathrm{PH-NC}}$ ($400\mu m$, 100 psi), with suggested cutoffs')
    ax_mag.loglog(f, mag, lw=1, color='k')
    ax_mag.set_ylabel(r'$|H_{\mathrm{PH-NC}}(f)|$')
    ax_mag.set_ylim(0.1, 100)
    ax_mag.axvline(f_cut, color='red', linestyle='--', lw=1)
    ...
    fig.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/400_100_H.pdf", dpi=600)
    plt.close()


def calibration_700_atm():
    root = 'data/20251014'
    fn = f'{root}/band_calib/700micron/atm.mat'
    OUTPUT_DIR = "figures/band_calib/700micron/atm"
    CAL_DIR = os.path.join(root, "tf_data")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
    ...
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/700_atm_band_calib.pdf", dpi=400)
    ...
    # FRF PH→NKD (x=PH, y=NKD)
    f, H, gamma2 = estimate_frf(ph1, nkd, FS, npsg=2**10)
    np.save(f"{CAL_DIR}/H_700_atm.npy", H)
    np.save(f"{CAL_DIR}/gamma2_700_atm.npy", gamma2)
    np.save(f"{CAL_DIR}/f_700_atm.npy", f)

    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/700_atm_H", "atm")
    ...
    fig.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/700_atm_H_band.pdf", dpi=600)
    plt.close()


def calibration_700_50():
    root = 'data/20251014'
    fn = f'{root}/band_calib/700micron/50psig.mat'
    OUTPUT_DIR = "figures/band_calib/700micron/50psig"
    CAL_DIR = os.path.join(root, "tf_data")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
    ...
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/700_50_band_calib.pdf", dpi=400)
    ...
    # FRF PH→NKD (x=PH, y=NKD)
    f, H, gamma2 = estimate_frf(ph1, nkd, FS, npsg=2**10)
    np.save(f"{CAL_DIR}/H_700_50.npy", H)
    np.save(f"{CAL_DIR}/gamma2_700_50.npy", gamma2)
    np.save(f"{CAL_DIR}/f_700_50.npy", f)

    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/700_50_H", "50psi")
    ...
    mag = np.abs(H); ph = np.unwrap(np.angle(H))
    fig, (ax_mag, ax_ph) = plt.subplots(2, 1, sharex=True, figsize=(6, 3), dpi=600)
    ax_mag.set_title(r'$H_{\mathrm{PH-NC}}$ ($700\mu m$, 50 psi), with suggested cutoffs')
    ax_mag.loglog(f, mag, lw=1, color='k')
    ax_mag.set_ylabel(r'$|H_{\mathrm{PH-NC}}(f)|$')
    ax_mag.set_ylim(0.1, 100)
    ax_mag.axvline(f_cut, color='red', linestyle='--', lw=1)
    ...
    fig.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/700_50_H_band.pdf", dpi=600)
    plt.close()

def calibration_700_100():
    root = 'data/20251014'
    fn = f'{root}/band_calib/700micron/100psig.mat'
    OUTPUT_DIR = "figures/band_calib/700micron/100psig"
    CAL_DIR = os.path.join(root, "tf_data")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
    ...
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/700_100_band_calib.pdf", dpi=400)
    ...
    # FRF PH→NKD (x=PH, y=NKD)
    f, H, gamma2 = estimate_frf(ph1, nkd, FS, npsg=2**10)
    np.save(f"{CAL_DIR}/H_700_100.npy", H)
    np.save(f"{CAL_DIR}/gamma2_700_100.npy", gamma2)
    np.save(f"{CAL_DIR}/f_700_100.npy", f)

    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/700_100_H", "100psi")
    ...
    mag = np.abs(H); ph = np.unwrap(np.angle(H))
    fig, (ax_mag, ax_ph) = plt.subplots(2, 1, sharex=True, figsize=(6, 3), dpi=600)
    ax_mag.set_title(r'$H_{\mathrm{PH-NC}}$ ($700\mu m$, 100 psi), with suggested cutoffs')
    ax_mag.loglog(f, mag, lw=1, color='k')
    ax_mag.set_ylabel(r'$|H_{\mathrm{PH-NC}}(f)|$')
    ax_mag.set_ylim(0.1, 100)
    ax_mag.axvline(f_cut, color='red', linestyle='--', lw=1)
    ...
    fig.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/700_100_H_band.pdf", dpi=600)
    plt.close()
