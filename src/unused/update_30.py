...
def main30_atm():
    root = 'data/30092025'
    fn = f'{root}/atm/Run2.mat'
    OUTPUT_DIR = "figures/cali_30/atm"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    dat = sio.loadmat(fn)
    ic(dat.keys())
    NC_atm, NH_atm = dat['channelData'].T
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.plot(np.arange(len(NH_atm)) / FS, NH_atm, label='PH')
    ax.plot(np.arange(len(NC_atm)) / FS, NC_atm, label='NC')
    ax.set_xlabel("Time [s]")
    ax.set_ylabel("Voltage [V]")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"{OUTPUT_DIR}/raw_signals_atm.pdf", dpi=400)
    # Compute spectra
    f_ph, Pyy_ph = compute_spec(FS, NH_atm)
    f_nc, Pyy_nc = compute_spec(FS, NC_atm)
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.semilogx(f_ph, f_ph * Pyy_ph, label='PH')
    ax.semilogx(f_nc, f_nc * Pyy_nc, label='NC')
    ax.set_xlabel("$f$ [Hz]")
    ax.set_ylabel(r"$f \phi_{pp}$")
    # ax.set_xlim(10, FS/2)
    ax.legend()
    fig.savefig(f"{OUTPUT_DIR}/raw_spectra_atm.pdf", dpi=400)

def main_30_atm_PH_callibration():
    root = 'data/30092025'
    fn = f'{root}/atm/nkd-ph_nofacilitynoise.mat'
    OUTPUT_DIR = "figures/cali_30/atm/PH-NKD"
    CAL_DIR = os.path.join(CALIB_BASE_DIR, "PH-NKD")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)

    # Load data (to Pa): col1 = PH, col2 = NKD
    nkd, ph = load_mat_to_pa(fn, 'channelData_nofacilitynoise', 'PH_300', 'NC_300')
    ...
    plot_transfer_PH(f, H, f"{OUTPUT_DIR}/H_atm_npsg{NPERSEG}", "atm")

    # Sanity: reconstruct PH from NKD using the inverse (should resemble PH)
    ph_hat = wiener_forward(ph, FS, f, H, gamma2)
    t = np.arange(len(ph_hat)) / FS
    plot_corrected_trace_PH(t, nkd, ph, ph_hat, f"{OUTPUT_DIR}/ph_recon_atm_npsg{NPERSEG}", "atm")
    ...
    dat = sio.loadmat(fn)
    NC_atm, NH_atm = dat['channelData'].T
    nkd_from_ph = apply_frf(NH_atm, FS, f, H)
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.plot(np.arange(len(nkd_from_ph)) / FS, nkd_from_ph, label='TF corrected PH')
    ax.plot(np.arange(len(NH_atm)) / FS, NH_atm, label='Raw PH')
    # ax.plot(np.arange(len(NC_atm)) / FS, NC_atm, label='Raw NC')
    ax.set_xlabel("Time [s]")
    ax.set_ylabel("Pressure [Pa]")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"{OUTPUT_DIR}/corrected_signals_atm.pdf", dpi=400)
    # save corrected signal
    np.save(f"{OUTPUT_DIR}/nkd_from_ph_atm.npy", nkd_from_ph)
    # Compute spectra
    f_ph, Pyy_ph = compute_spec(FS, NH_atm)
    f_nkd, Pyy_nkd = compute_spec(FS, nkd_from_ph)
    # f_nc, Pyy_nc = compute_spec(FS, NC_atm)
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.semilogx(f_nkd, f_nkd * Pyy_nkd, label='TF corrected PH')
    ax.semilogx(f_ph, f_ph * Pyy_ph, label='Raw PH')
    # ax.semilogx(f_nc, f_nc * Pyy_nc, label='Raw NC')
    ax.set_xlabel("$f$ [Hz]")
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"{OUTPUT_DIR}/corrected_spectra_atm.pdf", dpi=400)

def main_30_atm_NC_callibration():
    root = 'data/30092025'
    fn = f'{root}/atm/nkd-ns_nofacilitynoise.mat'
    OUTPUT_DIR = "figures/cali_30/atm/NC-NKD"
    CAL_DIR = os.path.join(CALIB_BASE_DIR, "NC-NKD")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)

    # Load data (to Pa): col1 = PH, col2 = NKD
    nkd, nc = load_mat_to_pa(fn, 'channelData', 'PH_300', 'NC_300')
    ...
    t = np.arange(len(ph_hat)) / FS
    plot_corrected_trace_PH(t, nkd, nc, ph_hat, f"{OUTPUT_DIR}/nc_recon_atm_npsg{NPERSEG}", "atm")
    ...
    dat = sio.loadmat(fn)
    NC_atm, NH_atm = dat['channelData'].T
    nkd_from_nc = apply_frf(NC_atm, FS, f, H)
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.plot(np.arange(len(nkd_from_nc)) / FS, nkd_from_nc, label='TF corrected PH')
    ax.plot(np.arange(len(NH_atm)) / FS, NH_atm, label='Raw PH')
    # ax.plot(np.arange(len(NC_atm)) / FS, NC_atm, label='Raw NC')
    ax.set_xlabel("Time [s]")
    ax.set_ylabel("Pressure [Pa]")
    ...
    fig.savefig(f"{OUTPUT_DIR}/corrected_signals_atm.pdf", dpi=400)
    # save corrected signal
    np.save(f"{OUTPUT_DIR}/nkd_from_nc_atm.npy", nkd_from_nc)
    # Compute spectra
    f_ph, Pyy_nc = compute_spec(FS, NH_atm)
    f_nkd, Pyy_nkd = compute_spec(FS, nkd_from_nc)
    # f_nc, Pyy_nc = compute_spec(FS, NC_atm)
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.semilogx(f_nkd, f_nkd * Pyy_nkd, label='TF corrected NC')
    ax.semilogx(f_ph, f_ph * Pyy_nc, label='Raw NC')
    # ax.semilogx(f_nc, f_nc * Pyy_nc, label='Raw NC')
    ax.set_xlabel("$f$ [Hz]")
    ax.set_ylabel(r"$f \phi_{pp}$")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"{OUTPUT_DIR}/corrected_nc_spectra_atm.pdf", dpi=400)

def main_30_atm_remove_facility_noise():
    root = 'data/30092025'
    fn = f'{root}/atm/Run2.mat'
    OUTPUT_DIR = "figures/cali_30/atm"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ...
    ic(dat.keys())
    NC_atm, NH_atm = dat['channelData'].T
    nkd_from_ph = np.load(f"{OUTPUT_DIR}/nkd_from_ph_atm.npy")
    nkd_from_nc = np.load(f"{OUTPUT_DIR}/nkd_from_nc_atm.npy")

    f, Pyy_ph = compute_spec(FS, nkd_from_ph)
    f, Pyy_nc = compute_spec(FS, nkd_from_nc)

    # plot raw signals (left) and spectra (right)
    fig, ax = plt.subplots(1, 2, figsize=(5, 2.5))
    ax[0].plot(np.arange(len(nkd_from_ph)) / FS, nkd_from_ph, label='TF corrected PH')
    ax[0].plot(np.arange(len(nkd_from_nc)) / FS, nkd_from_nc, label='TF corrected NC')
    ax[0].set_xlabel("Time [s]")
    ax[0].set_ylabel("Pressure [Pa]")
    # ax[0].legend()

    ax[1].semilogx(f, f * Pyy_ph, label='TF corrected PH')
    ax[1].semilogx(f, f * Pyy_nc, label='TF corrected NC')
    ax[1].set_xlabel("$f$ [Hz]")
    ax[1].set_ylabel(r"$f \phi_{pp}$")
    ax[1].legend()

    ax[1].set_xlim([10, 25000])
    fig.tight_layout()
    fig.savefig(f"{OUTPUT_DIR}/corrected_signals_and_spectra_atm.pdf", dpi=400)

    # Notch the common peaks using a digital comb filter
    # find the peaks to notch
    peaks_ph, _ = find_peaks(Pyy_ph, height=1e-4, distance=2)
    peaks_nc, _ = find_peaks(Pyy_nc, height=1e-4, distance=2)
    peaks = np.unique(np.concatenate([peaks_ph, peaks_nc]))
    freqs_to_notch = f[peaks]
    ic(freqs_to_notch)
    sos = design_notches(FS, freqs_to_notch, Q=10.0)
    nkd_from_ph_f = apply_notches(nkd_from_ph, sos)
    nkd_from_nc_f = apply_notches(nkd_from_nc, sos)
    f, Pyy_ph_f = compute_spec(FS, nkd_from_ph_f)
    f, Pyy_nc_f = compute_spec(FS, nkd_from_nc_f)

    # plot filtered signals (left) and spectra (right)
    fig, ax = plt.subplots(1, 2, figsize=(5, 2.5))
    ax[0].plot(np.arange(len(nkd_from_ph_f)) / FS, nkd_from_ph_f, label='Notched TF corrected PH')
    ax[0].plot(np.arange(len(nkd_from_nc_f)) / FS, nkd_from_nc_f, label='Notched TF corrected NC')
    ax[0].set_xlabel("Time [s]")
    ax[0].set_ylabel("Pressure [Pa]")
    # ax[0].legend()

    ax[1].semilogx(f, f * Pyy_ph_f, label='Notched TF corrected PH')
    ax[1].semilogx(f, f * Pyy_nc_f, label='Notched TF corrected NC')
    ax[1].set_xlabel("$f$ [Hz]")
    ax[1].set_ylabel(r"$f \phi_{pp}$")
    ax[1].legend()
    ax[1].set_xlim([10, 25000])
    fig.tight_layout()
    fig.savefig(f"{OUTPUT_DIR}/notched_signals_and_spectra_atm.pdf", dpi=400)
    

def main30_50psi():
    root = 'data/30092025'
    fn = f'{root}/50psi/Run1.mat'
    OUTPUT_DIR = "figures/cali_30/50psi"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    dat = sio.loadmat(fn)
    ic(dat.keys())
    NC_atm, NH_atm = dat['channelData_flow1'].T
    fig, ax = plt.subplots(1, 1, figsize=(5, 2.5), sharex=True)
    ax.plot(np.arange(len(NH_atm)) / FS, NH_atm, label='PH')
    ax.plot(np.arange(len(NC_atm)) / FS, NC_atm, label='NC')
    ax.set_xlabel("Time [s]")
...
def main_30_50psi_PH_callibration():
    root = 'data/30092025'
    fn = f'{root}/50psi/nkd-ph_nofacilitynoise.mat'
    OUTPUT_DIR = "figures/cali_30/50psi/PH-NKD"
    CAL_DIR = os.path.join(CALIB_BASE_DIR, "PH-NKD")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
    ...
    ph_hat = wiener_forward(ph, FS, f, H, gamma2)
    t = np.arange(len(ph_hat)) / FS
    plot_corrected_trace_PH(t, nkd, ph, ph_hat, f"{OUTPUT_DIR}/ph_recon_atm_npsg{NPERSEG}", "50psi")
...
def main_30_50psi_NC_callibration():
    root = 'data/30092025'
    fn = f'{root}/50psi/nkd-ns_nofacilitynoise.mat'
    OUTPUT_DIR = "figures/cali_30/50psi/NC-NKD"
    CAL_DIR = os.path.join(CALIB_BASE_DIR, "NC-NKD")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    os.makedirs(CAL_DIR, exist_ok=True)
...
def main_30_50psi_remove_facility_noise():
    root = 'data/30092025'
    fn = f'{root}/50psi/Run1.mat'
    OUTPUT_DIR = "figures/cali_30/50psi"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ...
    # find the peaks to notch
    # peaks_ph, _ = find_peaks(Pyy_ph, height=1e-4, distance=2)
    peaks_ph = f[np.argsort(Pyy_ph)[-20:]]  # take the 20 largest peaks only
    # peaks_nc, _ = find_peaks(Pyy_nc, height=1e-4, distance=2)
    peaks_nc = f[np.argsort(Pyy_nc)[-20:]]  # take the 20 largest peaks only
    freqs_to_notch = np.unique(np.concatenate([peaks_ph, peaks_nc]))[1:]  # skip DC
    # freqs_to_notch = f[peaks]
    ic(freqs_to_notch)
    sos = design_notches(FS, freqs_to_notch, Q=10.0)
